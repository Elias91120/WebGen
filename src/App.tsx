...
const IdeBar: React.FC<{ filename: string; accent?: string; variant?: 'badge' | 'window' }> = ({
  ...
  if (variant === 'window') {
    return (
      <div className="flex items-center gap-3 border-b border-white/10 bg-white/[0.045] px-5 py-3">
        <div className="flex items-center gap-1.5" aria-hidden="true">
          <span className="h-2.5 w-2.5 rounded-full bg-rose-400/80" />
          <span className="h-2.5 w-2.5 rounded-full bg-amber-300/80" />
          <span className="h-2.5 w-2.5 rounded-full bg-lime-300/80" />
        </div>
        <span className={`truncate rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-wide ${accentClass}`}>
          {filename}
        </span>
      </div>
    );
  ...
  return (
    <div className="px-5 pt-5 pb-1">
      <span className={`inline-flex rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-wide ${accentClass}`}>
        {filename}
      </span>
    </div>
  );
...
const App: React.FC = () => {
  ...
  return (
    ...
    <div className="min-h-screen bg-[#08090d] text-slate-100 font-sans relative overflow-x-hidden animate-in fade-in duration-700">
      ...
      {/* Navbar */}
      <nav className="fixed top-4 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl z-50 bg-[#0d1117]/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl transition-all duration-300">
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16 md:h-20">
            ...
            >
              <Logo variant="wordmark" className="group-hover:scale-[1.02] transition-transform duration-300" />
            </button>
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 w-full mt-2 bg-[#0d1117]/95 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-xl animate-in slide-in-from-top-2">
             <div className="flex flex-col space-y-3">
              <button onClick={() => scrollToSection('services')} className="text-left text-slate-200 p-2 hover:bg-white/5 hover:text-cyan-300 rounded-lg transition-colors">{t.nav.services}</button>
              <button onClick={() => scrollToSection('projets')} className="text-left text-slate-200 p-2 hover:bg-white/5 hover:text-cyan-300 rounded-lg transition-colors">{t.nav.projects}</button>
              <button onClick={() => scrollToSection('equipe')} className="text-left text-slate-200 p-2 hover:bg-white/5 hover:text-cyan-300 rounded-lg transition-colors">{t.nav.team}</button>
              <button onClick={() => scrollToSection('temoignages')} className="text-left text-slate-200 p-2 hover:bg-white/5 hover:text-cyan-300 rounded-lg transition-colors">{t.nav.reviews}</button>
              <button onClick={() => scrollToSection('contact')} className="bg-cyan-500 text-slate-950 p-3 rounded-lg font-bold text-center shadow-lg shadow-cyan-500/30">{t.nav.cta}</button>
             </div>
          </div>
        )}
      </nav>
      ...
      <main className="pt-24 md:pt-28 relative z-10">
        ...
        <section className="relative pt-12 md:pt-16 pb-12 md:pb-16">
           <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
             <div className="glass-panel p-6 md:p-10 rounded-3xl shadow-2xl max-w-5xl mx-auto border border-white/10">

               <div className="text-center space-y-5 md:space-y-7">

                 <div className="inline-flex items-center px-3 md:px-4 py-1.5 md:py-2 rounded-full bg-cyan-500/10 border border-cyan-400/30 text-cyan-200 text-xs uppercase tracking-wider mb-2 md:mb-4 animate-in slide-in-from-bottom-4 fade-in duration-700 backdrop-blur-sm">
                    {t.hero.badge}
                 </div>

                 <div className="text-3xl md:text-5xl lg:text-6xl font-display font-bold text-white leading-[1.05] tracking-tight drop-shadow-2xl">
                   <SplitText
                     text={t.hero.titleStart}
                     className="block mb-1 md:mb-2"
                     delay={50}
                   />
                   <span className="bg-clip-text text-transparent bg-gradient-to-r from-cyan-300 via-lime-300 to-cyan-300 block">
                     {t.hero.titleEnd}
                   </span>
                 </div>
                ...
                <p className="text-sm md:text-base text-slate-300 max-w-2xl mx-auto">
                 ...
                 <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4 pt-2 md:pt-4 animate-in slide-in-from-bottom-8 fade-in duration-1000 delay-500">
                   <button
                     onClick={() => scrollToSection('contact')}
                    className="w-full sm:w-auto px-6 md:px-8 py-3 md:py-4 bg-cyan-400 hover:bg-cyan-300 text-slate-950 rounded-xl font-semibold shadow-[0_0_24px_rgba(34,211,238,0.3)] transition-all transform hover:scale-[1.02] flex items-center justify-center gap-2 text-sm md:text-base"
                   >
                     {t.hero.ctaPrimary} <ArrowRight className="w-5 h-5" />
                   </button>
                   <button
                    onClick={() => scrollToSection('projets')}
                    className="w-full sm:w-auto px-6 md:px-8 py-3 md:py-4 bg-white/5 border border-white/15 hover:border-cyan-300/60 text-slate-100 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 text-sm md:text-base"
                   >
                     {t.hero.ctaSecondary}
                   </button>
                 </div>
             ...
             {/* STATS BAR */}
             <div className="max-w-5xl mx-auto mt-6 md:mt-8 grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
               {[
                 ...
                 <div key={i} className="glass-panel rounded-2xl p-4 text-center">
                  <div className="text-2xl md:text-3xl font-bold text-cyan-300 text-glow-cyan">{s.n}</div>
                  <div className="text-[11px] md:text-xs text-slate-400 uppercase tracking-widest mt-1">{s.l}</div>
                 </div>
               ))}
             </div>
           </div>
        ...
        {/* PROJECTS SECTION */}
        <section id="projets" className="py-14 md:py-20 relative bg-[#0d1117]/25">
           <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
             <div className="max-w-3xl mx-auto mb-10 md:mb-14 rounded-3xl text-center">
                <div className="text-xs text-cyan-300/80 mb-3 uppercase tracking-wide">{t.projects.path}</div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 md:gap-7">
                 ...
                 <div className="glass-panel rounded-3xl overflow-hidden transition-all duration-300 group flex flex-col border border-white/10 relative">
                   <IdeBar filename={t.projects.p1.tag} variant="window" />
                    <div className="p-6 md:p-7 relative flex flex-col flex-1">
                      ...
                      <div className="w-16 h-16 bg-white/5 border border-cyan-400/25 rounded-2xl flex items-center justify-center shadow-lg mb-5 relative z-10 overflow-hidden p-3 backdrop-blur-md">
                      ...
                      <p className="text-slate-300 leading-relaxed mb-5 relative z-10">{t.projects.p1.desc}</p>
                      <ul className="space-y-2 mb-8 relative z-10 flex-1">
                        <li className="text-sm text-slate-200 flex items-start gap-2"><CheckCircle2 className="w-4 h-4 mt-0.5 text-lime-300 flex-shrink-0" /> {t.projects.p1.result1}</li>
                        <li className="text-sm text-slate-200 flex items-start gap-2"><CheckCircle2 className="w-4 h-4 mt-0.5 text-lime-300 flex-shrink-0" /> {t.projects.p1.result2}</li>
                        <li className="text-sm text-slate-200 flex items-start gap-2"><CheckCircle2 className="w-4 h-4 mt-0.5 text-lime-300 flex-shrink-0" /> {t.projects.p1.result3}</li>
                      </ul>
                      <a
                        href={t.projects.p1.link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center justify-center gap-2 w-full py-3.5 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-cyan-400/40 rounded-xl font-semibold text-white transition-all relative z-10 group/btn"
                      >
                 ...
                 <div className="rounded-3xl overflow-hidden bg-gradient-to-br from-cyan-500/12 via-[#0d1117]/88 to-lime-500/10 backdrop-blur-2xl border border-cyan-400/35 flex flex-col transition-all duration-500 shadow-[0_18px_50px_-25px_rgba(34,211,238,0.45)] relative z-10">
                    ...
                    <div className="p-6 md:p-7 relative flex flex-col flex-1">
                      ...
                      <div className="w-16 h-16 bg-white/15 border border-white/25 rounded-2xl flex items-center justify-center mb-5 relative z-10 shadow-xl overflow-hidden p-1">
                         <img src={callKitchenImg} alt="CallKitchen" loading="lazy" className="w-full h-full object-cover rounded-xl" />
                      </div>
                      <h3 className="text-2xl font-bold text-white mb-3 relative z-10">{t.projects.p2.title}</h3>
                      <p className="text-slate-200 leading-relaxed mb-5 relative z-10">{t.projects.p2.desc}</p>
                      <ul className="space-y-2 mb-8 relative z-10 flex-1">
                        <li className="text-sm text-slate-100 flex items-start gap-2"><CheckCircle2 className="w-4 h-4 mt-0.5 text-lime-300 flex-shrink-0" /> {t.projects.p2.result1}</li>
                        <li className="text-sm text-slate-100 flex items-start gap-2"><CheckCircle2 className="w-4 h-4 mt-0.5 text-lime-300 flex-shrink-0" /> {t.projects.p2.result2}</li>
                        <li className="text-sm text-slate-100 flex items-start gap-2"><CheckCircle2 className="w-4 h-4 mt-0.5 text-lime-300 flex-shrink-0" /> {t.projects.p2.result3}</li>
                      </ul>
                      <a
                        href={t.projects.p2.link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center justify-center gap-2 w-full py-3.5 bg-cyan-400 hover:bg-cyan-300 text-slate-950 rounded-xl font-semibold shadow-lg shadow-cyan-500/25 transition-all relative z-10 group/btn"
                      >
                 ...
                 <div className="glass-panel rounded-3xl overflow-hidden transition-all duration-300 group flex flex-col border border-white/10 relative">
                    ...
                    <div className="p-6 md:p-7 relative flex flex-col flex-1">
                      ...
                      <div className="w-16 h-16 bg-violet-500/10 border border-violet-400/25 rounded-2xl flex items-center justify-center shadow-sm mb-5 relative z-10 overflow-hidden p-1">
                      ...
                      <p className="text-slate-300 leading-relaxed mb-5 relative z-10">{t.projects.p3.desc}</p>
                      <ul className="space-y-2 mb-8 relative z-10 flex-1">
                        <li className="text-sm text-slate-200 flex items-start gap-2"><CheckCircle2 className="w-4 h-4 mt-0.5 text-lime-300 flex-shrink-0" /> {t.projects.p3.result1}</li>
                        <li className="text-sm text-slate-200 flex items-start gap-2"><CheckCircle2 className="w-4 h-4 mt-0.5 text-lime-300 flex-shrink-0" /> {t.projects.p3.result2}</li>
                        <li className="text-sm text-slate-200 flex items-start gap-2"><CheckCircle2 className="w-4 h-4 mt-0.5 text-lime-300 flex-shrink-0" /> {t.projects.p3.result3}</li>
                      </ul>
                      <a
                        href={t.projects.p3.link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center justify-center gap-2 w-full py-3.5 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-violet-400/40 rounded-xl font-semibold text-white transition-all relative z-10 group/btn"
                      >
        ...
        <section className="py-16 md:py-24 relative bg-[#0d1117]/25">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mb-10 md:mb-14 flex flex-col items-center text-center">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-emerald-300/30 bg-emerald-300/[0.08] px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-200 backdrop-blur-sm">
              ...
              <p className="mt-4 max-w-2xl text-slate-300 md:text-lg">{t.openSource.subtitle}</p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
              ...
              <article className="relative overflow-hidden rounded-[2rem] border border-emerald-300/30 bg-gradient-to-br from-[#08130e] via-[#06110d] to-[#04140e] shadow-[0_30px_90px_-35px_rgba(52,211,153,0.55)] lg:col-span-3">
                ...
                <div className="relative z-10 flex h-full flex-col p-6 md:p-9">
                  <div className="mb-6 flex flex-col gap-5 sm:flex-row sm:items-center">
                    <PromptHubMark className="h-20 w-20 md:h-24 md:w-24 shrink-0" />
                    <div>
                      <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-emerald-300/85">
                        {t.openSource.p2.tag}
                      </div>
                      <h3 className="text-4xl md:text-5xl font-bold tracking-tight text-white">{t.openSource.p2.title}</h3>
                      <div className="mt-2 text-xs text-emerald-200/70">{t.openSource.p2.status}</div>
                    </div>
                  </div>

                  <p className="mb-7 max-w-2xl text-base md:text-lg leading-relaxed text-slate-200">{t.openSource.p2.desc}</p>

                  <div className="mb-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
                    {[t.openSource.p2.point1, t.openSource.p2.point2, t.openSource.p2.point3].map((point) => (
                      <div
                        key={point}
                        className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.04] p-4 text-sm text-slate-200 transition-colors hover:border-emerald-300/30 hover:bg-emerald-300/[0.07]"
                      >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex w-full sm:w-fit items-center justify-center gap-2 rounded-2xl bg-emerald-400 px-7 py-3.5 font-bold text-[#06110d] shadow-[0_22px_50px_-20px_rgba(52,211,153,0.8)] transition-all hover:bg-emerald-300 hover:shadow-[0_22px_60px_-15px_rgba(52,211,153,1)] group/link"
                  >
                    {t.openSource.p2.btn}
              ...
              <article className="relative flex flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-[#06110d]/85 backdrop-blur-xl transition-all hover:border-emerald-300/25 lg:col-span-2">
                ...
                <div className="flex flex-1 flex-col p-6 md:p-7">
                  <div className="mb-5 flex items-center gap-4">
                    <div className="relative flex h-14 w-14 items-center justify-center rounded-2xl border border-emerald-300/30 bg-gradient-to-br from-emerald-300/20 to-emerald-500/10 text-emerald-200 shadow-[0_8px_24px_-12px_rgba(52,211,153,0.6)]">
                      <Leaf className="h-6 w-6" />
                    </div>
                    <div>
                      <div className="mb-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-emerald-300/80">
                        {t.openSource.p1.tag}
                      </div>
                      <h3 className="text-2xl font-bold text-white">{t.openSource.p1.title}</h3>
                    </div>
                  </div>

                  <p className="mb-6 leading-relaxed text-slate-300">{t.openSource.p1.desc}</p>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-emerald-300/30 bg-emerald-300/[0.08] py-3.5 font-semibold text-emerald-100 transition-all hover:border-emerald-300/50 hover:bg-emerald-300/[0.14] group/link"
                  >
                    {t.openSource.p1.btn}
                    <ArrowRight className="h-4 w-4 transition-transform group-hover/link:translate-x-1" />
                  </a>
        ...
        {/* WHY US (Bento Grid) */}
        <section className="py-14 md:py-16 relative bg-[#0d1117]/25">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl mx-auto mb-12 bg-[#0d1117]/40 backdrop-blur-xl border border-white/5 p-6 rounded-3xl text-center shadow-2xl">
              <div className="text-xs font-mono text-cyan-300/80 mb-2">{t.whyUs.path}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               {/* Card 1 */}
               <div className="glass-panel col-span-1 md:col-span-2 rounded-3xl overflow-hidden hover:bg-[#0d1117]/85 transition-all duration-500 group hover:scale-[1.01] hover:shadow-2xl hover:shadow-cyan-500/15">
                  <IdeBar filename={t.whyUs.card1.file} />
                  <div className="p-8">
                    <div className="w-12 h-12 bg-cyan-400/15 border border-cyan-400/30 rounded-2xl flex items-center justify-center shadow-sm mb-6 text-cyan-300 group-hover:scale-110 transition-transform">
                       <Smartphone className="w-6 h-6" />
                    </div>
                    <h3 className="text-xl font-bold text-white mb-2 font-mono">{t.whyUs.card1.title}</h3>
                    <p className="text-slate-300 leading-relaxed">{t.whyUs.card1.desc}</p>
                  </div>
               </div>

               {/* Card 2 (highlighted) */}
               <div className="rounded-3xl overflow-hidden bg-gradient-to-br from-cyan-500/15 via-[#0d1117]/80 to-violet-500/10 border border-cyan-400/30 backdrop-blur-2xl text-white relative group transition-all duration-500 hover:scale-[1.01] hover:shadow-2xl hover:shadow-cyan-500/25">
                  <IdeBar filename={t.whyUs.card2.file} accent="lime" />
                  <div className="absolute top-2 right-0 -mt-4 -mr-4 w-24 h-24 bg-lime-400/20 rounded-full blur-2xl opacity-60 group-hover:opacity-100 transition-opacity"></div>
                  <div className="p-8 relative">
                    <div className="w-12 h-12 bg-lime-400/15 border border-lime-400/30 rounded-2xl flex items-center justify-center shadow-sm mb-6 text-lime-300 backdrop-blur-sm">
                       <Database className="w-6 h-6" />
                    </div>
                    <h3 className="text-xl font-bold mb-2 font-mono">{t.whyUs.card2.title}</h3>
                    <p className="text-slate-200 leading-relaxed">{t.whyUs.card2.desc}</p>
                  </div>
               </div>

               {/* Card 3 */}
               <div className="glass-panel rounded-3xl overflow-hidden hover:bg-[#0d1117]/85 transition-all duration-500 group hover:scale-[1.01] hover:shadow-2xl hover:shadow-violet-500/15">
                  <IdeBar filename={t.whyUs.card3.file} />
                  <div className="p-8">
                    <div className="w-12 h-12 bg-violet-400/15 border border-violet-400/30 rounded-2xl flex items-center justify-center shadow-sm mb-6 text-violet-300 group-hover:rotate-12 transition-transform">
                       <Store className="w-6 h-6" />
                    </div>
                    <h3 className="text-xl font-bold text-white mb-2 font-mono">{t.whyUs.card3.title}</h3>
                    <p className="text-slate-300 leading-relaxed">{t.whyUs.card3.desc}</p>
                  </div>
               </div>

               {/* Card 4 */}
               <div className="col-span-1 md:col-span-2 glass-panel rounded-3xl overflow-hidden hover:bg-[#0d1117]/85 transition-all duration-500 group hover:scale-[1.01] hover:shadow-2xl hover:shadow-cyan-500/15">
                  <IdeBar filename={t.whyUs.card4.file} />
                  <div className="p-8">
                    <div className="flex flex-col md:flex-row items-start gap-6">
                      <div className="w-12 h-12 bg-cyan-400/15 border border-cyan-400/30 rounded-2xl flex items-center justify-center shadow-sm text-cyan-300 flex-shrink-0">
                         <Users className="w-6 h-6" />
                      </div>
                      <div>
                        <h3 className="text-xl font-bold text-white mb-2 font-mono">{t.whyUs.card4.title}</h3>
                        <p className="text-slate-300 leading-relaxed">{t.whyUs.card4.desc}</p>
                      </div>
                    </div>
                  </div>
               </div>
        ...
        {/* STACK SECTION (NEW) */}
        <section className="py-12 md:py-16 relative bg-[#0d1117]/25">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-8">
              <div className="text-xs font-mono text-cyan-300/80 mb-2">{t.stack.path}</div>
              <h3 className="text-xl md:text-2xl font-display font-bold text-white">{t.stack.title}</h3>
              <p className="text-slate-400 text-sm mt-2 font-mono max-w-2xl mx-auto">{t.stack.subtitle}</p>
            </div>
            <div className="flex flex-wrap items-center justify-center gap-2.5 md:gap-3">
              {stackChips.map((chip, i) => (
                <div
                  key={i}
                  className="group flex items-center gap-2 px-4 py-2 rounded-full bg-[#0d1117]/70 border border-white/10 backdrop-blur-md font-mono text-sm text-slate-200 hover:border-cyan-400/50 hover:text-cyan-300 hover:shadow-[0_0_20px_rgba(34,211,238,0.2)] transition-all"
                >
        ...
        {/* SERVICES */}
        <section id="services" className="py-14 md:py-16 relative bg-[#0d1117]/25">
           <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="max-w-3xl mx-auto mb-12 bg-[#0d1117]/40 backdrop-blur-xl border border-white/5 p-6 rounded-3xl text-center shadow-2xl">
                 <div className="text-xs font-mono text-cyan-300/80 mb-2">{t.services.path}</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                 {/* Service 1 */}
                 <div className="glass-panel rounded-2xl overflow-hidden hover:border-cyan-400/40 transition-all duration-300 flex flex-col group hover:scale-[1.02] hover:-translate-y-1 hover:shadow-[0_10px_40px_-10px_rgba(34,211,238,0.35)]">
                    <IdeBar filename={lang === 'fr' ? 'Site vitrine' : 'Business website'} />
                    <div className="p-8 flex flex-col flex-1">
                      <div className="inline-block bg-white/10 text-white text-[11px] font-mono font-bold px-3 py-1 rounded-full w-fit mb-4 uppercase tracking-wider">{t.services.s1.badge}</div>
                      <h3 className="text-2xl font-display font-bold text-white mb-4">{t.services.s1.title}</h3>
                      <div className="mb-6">
                         <div className="text-3xl font-bold text-white font-mono">{t.services.s1.price}</div>
                         <div className="text-sm text-slate-400 font-mono">{t.services.s1.subPrice}</div>
                      </div>
                      <ul className="space-y-3 mb-8 flex-1">
                         {t.services.s1.features.map((item, i) => (
                           <li key={i} className="flex items-center gap-3 text-slate-200 text-sm">
                              <Zap className="w-4 h-4 text-cyan-300 flex-shrink-0 group-hover:text-lime-300 transition-colors" /> {item}
                           </li>
                         ))}
                      </ul>
                      <button onClick={() => handleServiceSelect('starter')} className="w-full py-3 border border-white/10 bg-white/5 rounded-xl font-mono font-bold text-white hover:bg-white/10 hover:border-cyan-400/50 transition-all">{t.services.s1.btn}</button>
                    </div>
                 </div>

                 {/* Service 2 (highlighted) */}
                 <div className="rounded-2xl overflow-hidden bg-gradient-to-br from-cyan-500/10 via-[#0d1117]/85 to-violet-500/10 border border-cyan-400/40 relative transform md:-translate-y-4 hover:md:-translate-y-5 hover:scale-[1.03] transition-all duration-300 shadow-[0_0_40px_rgba(34,211,238,0.25)] hover:shadow-[0_0_80px_rgba(34,211,238,0.55)] flex flex-col z-10">
                    ...
                    <div className="p-8 flex flex-col flex-1">
                      ...
                      <div className="mb-6">
                         <div className="text-3xl font-bold text-cyan-300 font-mono">{t.services.s2.price}</div>
                         <div className="text-sm text-slate-300 font-mono">{t.services.s2.subPrice}</div>
                      </div>
                      <ul className="space-y-3 mb-8 flex-1">
                         {t.services.s2.features.map((item, i) => (
                           <li key={i} className="flex items-center gap-3 text-slate-100 text-sm">
                              <CheckCircle2 className="w-4 h-4 text-lime-300 flex-shrink-0" /> {item}
                           </li>
                         ))}
                      </ul>
                      <button onClick={() => handleServiceSelect('custom')} className="w-full py-3 bg-cyan-400 hover:bg-cyan-300 text-slate-950 rounded-xl font-mono font-bold transition-colors shadow-lg shadow-cyan-500/30">{t.services.s2.btn}</button>
                    </div>
                 </div>

                 {/* Service 3 */}
                 <div className="glass-panel rounded-2xl overflow-hidden hover:border-violet-400/40 transition-all duration-300 flex flex-col group hover:scale-[1.02] hover:-translate-y-1 hover:shadow-[0_10px_40px_-10px_rgba(167,139,250,0.4)]">
                    <IdeBar filename={lang === 'fr' ? 'Refonte complète' : 'Complete redesign'} />
                    <div className="p-8 flex flex-col flex-1">
                      <div className="inline-block bg-violet-400/20 text-violet-300 text-[11px] font-mono font-bold px-3 py-1 rounded-full w-fit mb-4 uppercase tracking-wider">{t.services.s3.badge}</div>
                      <h3 className="text-2xl font-display font-bold text-white mb-4">{t.services.s3.title}</h3>
                      <div className="mb-6">
                         <div className="text-3xl font-bold text-white font-mono">{t.services.s3.price}</div>
                         <div className="text-sm text-slate-400 font-mono">{t.services.s3.subPrice}</div>
                      </div>
                      <ul className="space-y-3 mb-8 flex-1">
                         {t.services.s3.features.map((item, i) => (
                           <li key={i} className="flex items-center gap-3 text-slate-200 text-sm">
                              <RefreshCw className="w-4 h-4 text-violet-300 flex-shrink-0 group-hover:rotate-180 transition-transform duration-500" /> {item}
                           </li>
                         ))}
                      </ul>
                      <button onClick={() => handleServiceSelect('redesign')} className="w-full py-3 border border-white/10 bg-white/5 rounded-xl font-mono font-bold text-white hover:bg-white/10 hover:border-violet-400/50 transition-all">{t.services.s3.btn}</button>
                    </div>
                 </div>
              </div>
              ...
              {/* Maintenance Add-on */}
              <div
                className="mt-12 max-w-2xl mx-auto glass-panel p-6 rounded-2xl border border-cyan-400/30 flex flex-col sm:flex-row items-center justify-between gap-6 hover:bg-[#0d1117]/85 transition-all duration-300 group hover:shadow-[0_10px_30px_-10px_rgba(34,211,238,0.35)] hover:-translate-y-1 cursor-pointer"
                onClick={() => handleServiceSelect('maintenance_only')}
              >
                 <div className="flex items-start gap-4">
                    <div className="p-3 bg-cyan-400/15 rounded-xl text-cyan-300 group-hover:scale-110 transition-transform">
                       <ShieldCheck className="w-6 h-6" />
                    </div>
                    <div>
                       <h4 className="text-white font-bold text-lg font-mono">{t.services.maintenanceTitle}</h4>
                       <p className="text-slate-300 text-sm max-w-sm">
                          {t.services.maintenanceDesc} <span className="text-lime-300 font-bold font-mono">{t.services.maintenancePrice}</span>.
                       </p>
                    </div>
                 </div>
                 <button className="whitespace-nowrap px-6 py-2 bg-white/5 border border-white/10 hover:bg-white/10 rounded-full text-sm font-mono font-bold text-white transition-all">
                    + add option
                 </button>
              </div>
           </div>
        ...
        {/* TEAM SECTION */}
        <section id="equipe" className="py-16 md:py-20 relative bg-[#0d1117]/25">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl mx-auto mb-16 bg-[#0d1117]/40 backdrop-blur-xl border border-white/5 p-8 rounded-3xl text-center shadow-2xl">
              <div className="text-xs font-mono text-cyan-300/80 mb-2">{t.team.path}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              {/* Member 1: Elias */}
              <div className="glass-panel rounded-2xl overflow-hidden hover:bg-[#0d1117]/85 transition-all duration-300 group text-center flex flex-col hover:scale-[1.02] hover:shadow-[0_20px_40px_-10px_rgba(34,211,238,0.4)]">
                 <IdeBar filename={t.team.roles.elias} />
                 <div className="p-6 flex flex-col items-center">
                   <div className="relative w-28 h-28 mb-6 group-hover:scale-105 transition-transform p-1 rounded-full border-2 border-cyan-400/50 shadow-[0_0_20px_rgba(34,211,238,0.3)]">
                      <img
                        src={eliasImg}
                        alt="Elias Eloumi"
                  ...
                  <p className="text-cyan-300 text-xs mb-4 flex items-center gap-1 justify-center"><Sparkles className="w-3 h-3" />{t.team.roles.elias}</p>
                   <p className="text-slate-300 text-sm mb-6 max-w-xs leading-relaxed">{t.team.roles.eliasDesc}</p>
                   <a
                     href="https://www.linkedin.com/in/elias-eloumi/"
                     target="_blank"
                     rel="noopener noreferrer"
                    className="mt-auto inline-flex items-center gap-2 px-4 py-2 bg-sky-600 hover:bg-sky-500 rounded-xl text-white text-sm font-semibold transition-colors"
                   >
              ...
              {/* Member 2: Noam */}
              <div className="glass-panel rounded-2xl overflow-hidden hover:bg-[#0d1117]/85 transition-all duration-300 group text-center flex flex-col hover:scale-[1.02] hover:shadow-[0_20px_40px_-10px_rgba(163,230,53,0.4)]">
                 <IdeBar filename={t.team.roles.noam} accent="lime" />
                 <div className="p-6 flex flex-col items-center">
                   <div className="relative w-28 h-28 mb-6 group-hover:scale-105 transition-transform p-1 rounded-full border-2 border-lime-400/50 shadow-[0_0_20px_rgba(163,230,53,0.3)]">
                      <img
                        src={noamImg}
                        alt="Noam Leclappart"
                        className="w-full h-full rounded-full object-cover"
                  ...
                  <p className="text-lime-300 text-xs mb-4 flex items-center gap-1 justify-center"><Database className="w-3 h-3" />{t.team.roles.noam}</p>
                   <p className="text-slate-300 text-sm mb-6 max-w-xs leading-relaxed">{t.team.roles.noamDesc}</p>
                   <a
               ...
               {/* Member 3: Charles */}
              <div className="glass-panel rounded-2xl overflow-hidden hover:bg-[#0d1117]/85 transition-all duration-300 group text-center flex flex-col hover:scale-[1.02] hover:shadow-[0_20px_40px_-10px_rgba(167,139,250,0.4)]">
                 <IdeBar filename={t.team.roles.charles} />
                 <div className="p-6 flex flex-col items-center">
                   <div className="relative w-28 h-28 mb-6 group-hover:scale-105 transition-transform p-1 rounded-full border-2 border-violet-400/50 shadow-[0_0_20px_rgba(167,139,250,0.3)]">
                      <img
                        src={charlesImg}
                        alt="Charles Garbus"
                        className="w-full h-full rounded-full object-cover"
                  ...
                  <p className="text-violet-300 text-xs mb-4 flex items-center gap-1 justify-center"><Users className="w-3 h-3" />{t.team.roles.charles}</p>
                   <p className="text-slate-300 text-sm mb-6 max-w-xs leading-relaxed">{t.team.roles.charlesDesc}</p>
                   <a
        ...
        {/* SOCIAL PROOF */}
        <section id="temoignages" className="py-16 md:py-20 relative bg-[#0d1117]/25">
           <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="max-w-3xl mx-auto mb-16 bg-[#0d1117]/40 backdrop-blur-xl border border-white/5 p-8 rounded-3xl text-center shadow-2xl">
                <div className="text-xs text-cyan-300/80 mb-2 uppercase tracking-wide">{t.reviews.path}</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 ...
                 <div className="glass-panel rounded-2xl overflow-hidden">
                   <IdeBar filename={t.reviews.path} accent="lime" />
                    <div className="p-6">
                      <div className="flex gap-1 text-lime-300 mb-3 items-center">
                        <span className="text-[11px] mr-2">5/5</span>
                         {[1,2,3,4,5].map(i => <Star key={i} className="w-4 h-4 fill-current" />)}
                      </div>
                      <p className="text-slate-100 mb-6 italic font-medium">"{t.reviews.r1}"</p>
                 ...
                 <div className="glass-panel rounded-2xl overflow-hidden border-lime-400/30">
                   <IdeBar filename={t.reviews.path} accent="lime" />
                    <div className="p-6">
                      <div className="flex gap-1 text-lime-300 mb-3 items-center">
                         <span className="text-[11px] mr-2">5/5</span>
                      ...
                      <p className="text-slate-100 mb-6 italic font-medium">"{t.reviews.r2}"</p>
                      <div className="flex items-center gap-3">
                 ...
                 <div className="glass-panel rounded-2xl overflow-hidden hidden lg:block">
                   <IdeBar filename={t.reviews.path} accent="lime" />
                    <div className="p-6">
                      ...
                      <p className="text-slate-100 mb-6 italic font-medium">"{t.reviews.r3}"</p>
                      <div className="flex items-center gap-3">
        ...
        {/* CONTACT SECTION */}
        <section id="contact" className="py-16 md:py-20 relative bg-[#0d1117]/25">
           <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="glass-panel rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row border border-white/10">
                 ...
                 <div className="p-8 md:p-12 md:w-3/5 relative">
                   <div className="text-xs text-cyan-300/80 mb-2 uppercase tracking-wide">{t.contact.path}</div>
                    ...
                    {formStatus === 'success' ? (
                       <div className="h-full flex flex-col items-center justify-center text-center animate-in fade-in py-12">
                          <div className="w-20 h-20 bg-lime-400/15 text-lime-300 rounded-full flex items-center justify-center mb-6 border border-lime-400/30 glow-lime">
                          ...
                          <div className="p-4 bg-cyan-500/10 border border-cyan-400/30 rounded-xl text-sm text-cyan-200">
                       ...
                       <form onSubmit={handleSubmit} className="space-y-6">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                             <div className="group">
                               <label className="block text-xs font-semibold text-slate-300 mb-2 uppercase tracking-wider">{t.contact.form.name}</label>
                                <input
                                   name="name"
                                   required
                                   type="text"
                                   placeholder="John Doe"
                                   className="w-full px-4 py-3.5 rounded-xl bg-[#0d1117]/70 backdrop-blur-md border border-white/10 focus:border-cyan-400 focus:bg-white/5 focus:ring-2 focus:ring-cyan-400/30 outline-none transition-all duration-300 text-white placeholder:text-slate-500 shadow-inner"
                                />
                             </div>
                             <div className="group">
                                <label className="block text-xs font-mono font-semibold text-slate-300 mb-2 uppercase tracking-wider">{t.contact.form.email}</label>
                                <input
                                   name="email"
                                   required
                                   type="email"
                                   placeholder="john@example.com"
                                   className="w-full px-4 py-3.5 rounded-xl bg-[#0d1117]/70 backdrop-blur-md border border-white/10 focus:border-cyan-400 focus:bg-white/5 focus:ring-2 focus:ring-cyan-400/30 outline-none transition-all duration-300 text-white placeholder:text-slate-500 font-mono shadow-inner"
                                />
                             </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="group">
                               <label className="block text-xs font-mono font-semibold text-slate-300 mb-2 uppercase tracking-wider">{t.contact.form.type}</label>
                               <div className="relative">
                                 <select name="type" className="w-full px-4 py-3.5 rounded-xl bg-[#0d1117]/70 backdrop-blur-md border border-white/10 focus:border-cyan-400 focus:bg-white/5 focus:ring-2 focus:ring-cyan-400/30 outline-none transition-all duration-300 text-white appearance-none cursor-pointer font-mono shadow-inner">
                            ...
                            <div className="group">
                               <label className="block text-xs font-mono font-semibold text-slate-300 mb-2 uppercase tracking-wider">{t.contact.form.serviceInterest}</label>
                               <div className="relative">
                                 <select name="service" className="w-full px-4 py-3.5 rounded-xl bg-[#0d1117]/70 backdrop-blur-md border border-white/10 focus:border-cyan-400 focus:bg-white/5 focus:ring-2 focus:ring-cyan-400/30 outline-none transition-all duration-300 text-white appearance-none cursor-pointer font-mono shadow-inner">
                          ...
                          <div className="group">
                             <label className="block text-xs font-semibold text-slate-300 mb-2 uppercase tracking-wider">{t.contact.form.message}</label>
                             <textarea
                                name="message"
                                required
                                rows={4}
                                placeholder="Tell us about your project, goals and deadline..."
                                className="w-full px-4 py-3.5 rounded-xl bg-[#0d1117]/70 backdrop-blur-md border border-white/10 focus:border-cyan-400 focus:bg-white/5 focus:ring-2 focus:ring-cyan-400/30 outline-none transition-all duration-300 text-white placeholder:text-slate-500 shadow-inner resize-none"
                             ></textarea>
                          </div>
                          ...
                          <button
                             ...
                             className="w-full py-4 mt-2 bg-cyan-400 hover:bg-cyan-300 text-slate-950 rounded-xl font-semibold shadow-[0_0_30px_rgba(34,211,238,0.4)] hover:shadow-[0_0_50px_rgba(34,211,238,0.6)] transition-all duration-300 transform hover:scale-[1.01] flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                          >
                 ...
                 <div className="bg-gradient-to-br from-cyan-500/15 via-[#0d1117] to-violet-500/10 p-8 md:p-12 md:w-2/5 text-slate-100 flex flex-col items-center justify-center text-center backdrop-blur-xl relative overflow-hidden border-t md:border-t-0 md:border-l border-white/10">
                    ...
                    <div className="relative z-10 flex flex-col items-center">
                       ...
                       <div className="relative w-32 h-32 mb-6 p-1 rounded-full border-2 border-lime-400/50 shadow-[0_0_24px_rgba(163,230,53,0.3)]">
                         <img
                           src={charlesImg}
                           alt="Charles Garbus"
                           className="w-full h-full rounded-full object-cover"
                       ...
                       <p className="text-slate-300 text-sm mb-8">{t.contact.direct.subtitle}</p>
                       ...
                       <div className="mt-4 text-xs font-mono text-slate-500">{t.contact.direct.phone}</div>
                    </div>
                 </div>
              </div>
           </div>
        ...
        {/* PRE-FOOTER CTA BAND */}
        <section className="relative py-10 md:py-12 bg-[#0d1117]/25">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-r from-[#0d1117]/85 via-[#0d1117]/70 to-[#0d1117]/85 backdrop-blur-xl px-6 py-7 md:px-10 md:py-8 shadow-[0_24px_60px_-30px_rgba(34,211,238,0.35)]">
              <div className="pointer-events-none absolute -left-24 -top-24 h-56 w-56 rounded-full bg-cyan-400/15 blur-3xl" aria-hidden="true" />
              <div className="pointer-events-none absolute -right-24 -bottom-24 h-56 w-56 rounded-full bg-lime-400/10 blur-3xl" aria-hidden="true" />

              <div className="relative z-10 flex flex-col items-center gap-5 text-center md:flex-row md:items-center md:justify-between md:text-left">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-lg md:text-xl font-display font-bold text-white">
                      {lang === 'fr' ? "Prêt à lancer votre projet ?" : "Ready to start your project?"}
                    </div>
                    <p className="text-sm text-slate-400 mt-1">
                      {lang === 'fr'
                        ? "Réponse sous 24h. Devis clair. Pas de jargon."
                        : "Reply within 24h. Clear quote. No jargon."}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                  <button
                    onClick={() => scrollToSection('contact')}
                    className="inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-400 hover:bg-cyan-300 text-slate-950 px-6 py-3 font-semibold shadow-[0_0_24px_rgba(34,211,238,0.3)] transition-all"
                  >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-white px-6 py-3 font-semibold transition-all"
                  >
                    WhatsApp
                  </a>
                </div>
        ...
        <div className="md:hidden fixed bottom-4 left-4 right-4 z-40">
          <button
            onClick={() => scrollToSection('contact')}
            className="w-full py-3 bg-cyan-400 text-slate-950 rounded-xl font-semibold shadow-lg shadow-cyan-500/30"
          >
            {t.nav.cta}
          </button>
        </div>
      ...
      {/* FOOTER */}
      <footer className="relative bg-[#0d1117]/90 border-t border-white/5 py-10 backdrop-blur-xl mt-12">
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-3">
              <Mark className="w-7 h-7" />
              <div className="text-sm">
                <span className="text-white font-semibold">3geeks</span>
                <span className="text-slate-500"> © {new Date().getFullYear()} - </span>
                <span className="text-lime-300">{t.footer.tagline}</span>
              </div>
            </div>
            <div className="flex gap-5 text-sm font-mono text-slate-400 items-center">
